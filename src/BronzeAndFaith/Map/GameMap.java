...
public class GameMap {
	...
	private static HashMap<Point, Tile> matrix = new HashMap<Point, Tile>(); //Tilemap with enum references
	private static HashMap<Point, GameTile> tileMap = new HashMap<Point, GameTile>();
	 ...
	 */
	public static Tile getTileType(int x, int y) {	
		Point p = new Point(x,y);
	...
	public static void setGameTile(int x, int y, GameTile tile) {
		if (x > width || y > height || x < 0 || y < 0)
			return;
	...
	private void cleanTerrain(){
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				...
				int count = 0;
				Tile ownTile = getTileType((int)p.x,(int)p.y);
				Tile otherTile = ownTile;
				for (Point point : neighbors){
					Tile t = getTileType((int)point.x,(int)point.y);
					if(ownTile == t){
						count++;
					} else {
						otherTile = t;
	 ...
	 */
	static ArrayList<Point> randomExpansion(Point start, int size, int iterations){
	...
	private void createSwamp(){
		System.out.println("Creating swamp...");
		Swamp swamp = new Swamp(riverPoints);
		swamps.add(swamp);
	}
	...
	public static Point randomNeighbor(Point self){
		int x = self.x;
		int y = self.y;
